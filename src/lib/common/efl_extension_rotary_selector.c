...
_selector_update(Eext_Rotary_Selector_Data *rsd, Eina_Bool page_changed)
{
   ...
   if ((rsd->state == EEXT_ROTARY_SELECTOR_SELECTOR_STATE_NORMAL) && rsd->selector_bg_image_color.normal_color.set_flag)
     ROTARY_SELECTOR_EVAS_OBJECT_COLOR_SET(elm_object_part_content_get(rsd->rotary_selector, "selector,bg_image"), rsd, selector_bg_image_color, normal_color);
   else if ((rsd->state == EEXT_ROTARY_SELECTOR_SELECTOR_STATE_PRESSED) && rsd->selector_bg_image_color.pressed_color.set_flag)
     ROTARY_SELECTOR_EVAS_OBJECT_COLOR_SET(elm_object_part_content_get(rsd->rotary_selector, "selector,bg_image"), rsd, selector_bg_image_color, pressed_color);
...
_selector_content_update(Eext_Rotary_Selector_Data *rsd)
{
   ...
   Eina_List *l = NULL;
   Eext_Rotary_Selector_Item *item = NULL;
   Evas_Object *content_icon, *custom_content;
        ...
        if (item->index == rsd->selected_index)
          {
             if ((item->state == EEXT_ROTARY_SELECTOR_ITEM_STATE_NORMAL) && (item->selector_content.normal))
               {
                  custom_content = elm_object_part_content_unset(elm_object_part_content_get(rsd->rotary_selector, "content"), "custom_content");
                  if (custom_content)
                    {
                       evas_object_hide(custom_content);
                    }
               ...
               {
                  content_icon = elm_object_part_content_unset(elm_object_part_content_get(rsd->rotary_selector, "content"), "selector,icon");
                  if (content_icon)
                    {
             ...
             if ((item->state == EEXT_ROTARY_SELECTOR_ITEM_STATE_NORMAL) && item->item_bg_image_color.normal_color.set_flag)
               {
                  ROTARY_SELECTOR_EVAS_OBJECT_COLOR_SET(elm_object_part_content_get(item->rotary_selector, "item,bg_image"), item, item_bg_image_color, normal_color);
               }
             else if ((item->state == EEXT_ROTARY_SELECTOR_ITEM_STATE_PRESSED) && item->item_bg_image_color.pressed_color.set_flag)
               {
                  ROTARY_SELECTOR_EVAS_OBJECT_COLOR_SET(elm_object_part_content_get(item->rotary_selector, "item,bg_image"), item, item_bg_image_color, pressed_color);
               }

             if ((item->state == EEXT_ROTARY_SELECTOR_ITEM_STATE_NORMAL) && item->item_icon_color.normal_color.set_flag)
               {
                  ROTARY_SELECTOR_EVAS_OBJECT_COLOR_SET(elm_object_part_content_get(item->rotary_selector, "item,icon"), item, item_icon_color, normal_color);
               }
             else if ((item->state == EEXT_ROTARY_SELECTOR_ITEM_STATE_PRESSED) && item->item_icon_color.pressed_color.set_flag)
               {
                  ROTARY_SELECTOR_EVAS_OBJECT_COLOR_SET(elm_object_part_content_get(item->rotary_selector, "item,icon"), item, item_icon_color, pressed_color);
               }

             if ((item->state == EEXT_ROTARY_SELECTOR_ITEM_STATE_NORMAL) && item->selector_icon_color.normal_color.set_flag)
               {
                  ROTARY_SELECTOR_EVAS_OBJECT_COLOR_SET(elm_object_part_content_get(item->rotary_selector, "selector_icon"), item, selector_icon_color, normal_color);
               }
             else if ((item->state == EEXT_ROTARY_SELECTOR_ITEM_STATE_PRESSED) && item->selector_icon_color.pressed_color.set_flag)
               {
                  ROTARY_SELECTOR_EVAS_OBJECT_COLOR_SET(elm_object_part_content_get(item->rotary_selector, "selector_icon"), item, selector_icon_color, pressed_color);
               }

             if ((item->state == EEXT_ROTARY_SELECTOR_ITEM_STATE_NORMAL) && item->custom_content_color.normal_color.set_flag)
               {
                  ROTARY_SELECTOR_EVAS_OBJECT_COLOR_SET(elm_object_part_content_get(item->rotary_selector, "selector,content"), item, custom_content_color, normal_color);
               }
             else if ((item->state == EEXT_ROTARY_SELECTOR_ITEM_STATE_PRESSED) && item->custom_content_color.pressed_color.set_flag)
               {
                  ROTARY_SELECTOR_EVAS_OBJECT_COLOR_SET(elm_object_part_content_get(item->rotary_selector, "selector,content"), item, custom_content_color, pressed_color);
               }
          }
        i++;
     }
